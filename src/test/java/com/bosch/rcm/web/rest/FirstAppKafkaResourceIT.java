...
import com.bosch.rcm.config.KafkaProperties;
...
class FirstAppKafkaResourceIT {
    ...
    void setup() {
        ...
        FirstAppKafkaResource kafkaResource = new FirstAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/first-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/first-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
